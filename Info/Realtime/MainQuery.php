...
function availablePilotGrade() {
    $sql="SELECT CHPILOTGRADE,CHPILOTCLASS,COUNT(*) FROM pilotplan.t_data_pilot WHERE CHPILOTSTATE=1 GROUP BY CHPILOTGRADE,CHPILOTCLASS ORDER BY CHPILOTGRADE,CHPILOTCLASS;";
    $result=db_Query($sql);
    echo "\"apg\":[";
...
function PilotState() {
    $sql="SELECT CHPILOTSTATE,COUNT(*) FROM pilotplan.t_data_pilot GROUP BY CHPILOTSTATE ORDER BY CHPILOTSTATE;";
    $result=db_Query($sql);
    echo "\"ps\":[";
    $row = mysqli_fetch_assoc($result);
